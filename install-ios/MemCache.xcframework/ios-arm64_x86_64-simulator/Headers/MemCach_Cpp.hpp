...
template <typename T> int put(const std::string& key, const T& value) = delete; // 禁止使用未特化版本的put函数
...
template <typename T> int put(const std::vector<std::pair<std::string, T>>& kvs) = delete;
...
template <typename T> optional<T> get(const std::string& key) = delete; // 禁止使用未特化版本的get函数
